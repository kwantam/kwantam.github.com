...
iterate takes a function and an initial argument and creates an infinite
list of applications of the function to the previous value of the list,
with the initial argument itself as the base case. Using this, we can
make a function that returns a (possibly) finite list by ending when we
see the initial argument again.
...
supplied value, in our case 0 or 1. selX generates a list of functions
that match a supplied list against some subsequence of length two of
the argument.
...
We have no choice but to escalate our abuse of flip et al:
...
one that we detect as such). In other words, we've got a canary for our
coal mine.
...
that our Eq, Ord, and Show instances only work on the names, not on the
